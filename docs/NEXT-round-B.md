...
> ---
>
> **⚠️ 范围 C 对账注（t7 追加，只加这一段，正文不动）**：本文件的待办**已在范围 C 关闭或改判** ——
> §1（A0-2 去盲 / 自检段 / 根并集）、§2（12 个零断言符号）、§4（双格式产物）、§5（BUG-6）**都已落地**；
> §3 表格里的数字有两处已过期：`F2` 那条「`ui-kit` 53 → 56」的绝对值整体偏 3（实测「控件 DOM 契约」段
> 是 **62 → 65**；增量 +3 与所列断言名是对的），`F8` 那条「12 个被跟踪文件工作区是 CRLF」在本机
> `git ls-files --eol` 实测是 **`w/crlf` = 3**（这个数随检出/平台变，**写命令、别写死数字**）。
> 当前现值、完成项、延期项与下一轮入口见 [`docs/WAVE-C-closeout.md`](WAVE-C-closeout.md)。
